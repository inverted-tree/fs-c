...
class TemporalRedundancyHandler(output: Option[String], d: ChunkIndex)
    ...
    def handle(f: File): Unit = {
        lock.synchronized {
            ...
            typeMap += (currentFileType -> (
              typeMap(currentFileType)._1 + currentRealSize,
              typeMap(currentFileType)._2 + currentFileSize
            ))
            typeMap += ("ALL" -> (
            ...
            sizeCategoryMap += (currentSizeCategory -> (
              sizeCategoryMap(currentSizeCategory)._1 + currentRealSize,
              sizeCategoryMap(currentSizeCategory)._2 + currentFileSize
            ))
            sizeCategoryMap += ("ALL" -> (
    ...
    override def quit(): Unit = {
        output match {
            ...
            case None =>
                println("Temporal Redundancy Results")
    ...
    ): scala.collection.Seq[de.pc2.dedup.chunker.Chunk] = {
