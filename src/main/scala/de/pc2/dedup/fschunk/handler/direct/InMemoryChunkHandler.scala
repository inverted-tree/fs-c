...
/** Handler to do a in-memory analysis of the trace.
  */
class InMemoryChunkHandler(
    silent: Boolean,
    d: ChunkIndex,
    chunkerName: Option[String]
    ...
    override def report(): Unit = {
        lock.synchronized {
            ...
            if (totalFileSize > 0) {
                msg.append(
                  " (%.2f%%)".format(100.0 * totalRedundancy / totalFileSize)
                )
            }
            ...
            if (seconds > 0) {
                msg.append(
                  "Throughput: %s/s".format(
                    StorageUnit(totalFileSize / seconds)
                  )
                )
            } else {
    ...
    private def formatOutput(
        ...
        val illegalFile =
            patchSize < 0 || redundancy < 0 || f.fileSize != chunkFileSize
            ...
            if (illegalFile) {
                msg.append(
                  "\nChunks size: %s (%d Byte)%n".format(
                    StorageUnit(chunkFileSize),
                    chunkFileSize
                  )
                )
            }
            msg.append(
              "Redundancy: %s  (%d Byte)".format(
                StorageUnit(redundancy),
                redundancy
              )
            )
            if (chunkFileSize > 0) {
                msg.append(
                  " (%.2f%%)".format(100.0 * redundancy / chunkFileSize)
                )
            }
            msg.append(
              "%nPatch Size: %s (%d Byte)".format(
                StorageUnit(chunkFileSize - redundancy),
                chunkFileSize - redundancy
              )
            )
        }
        if (illegalFile) {
    ...
    private def gatherAllFileChunks(
        f: de.pc2.dedup.chunker.File
    ): Seq[de.pc2.dedup.chunker.Chunk] = {

        val allFileChunks: Seq[Chunk] =
