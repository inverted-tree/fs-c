...
object HarnikEstimationScanHandler {
    def outputTemporalScanResult(
        sample: HarnikEstimationSample,
        totalEstimator: HarnikEstimationSampleCounter,
        firstGenerationEstimator: HarnikEstimationSampleCounter
    ): Unit = {

        val secondTotalSize =
            totalEstimator.totalChunkSize - firstGenerationEstimator.totalChunkSize
        if (
          totalEstimator.deduplicationRatio.isNaN || firstGenerationEstimator.deduplicationRatio.isNaN
        ) {
            outputScanResult(sample, secondTotalSize, Double.NaN)
        } else {
            val totalChunkSize =
                totalEstimator.totalChunkSize * totalEstimator.deduplicationRatio
            val firstGenChunkSize =
                firstGenerationEstimator.totalChunkSize * firstGenerationEstimator.deduplicationRatio
            val secondGenChunkSize = totalChunkSize - firstGenChunkSize
            val secondGenDeduplicationRatio =
                secondGenChunkSize.toDouble / secondTotalSize.toDouble
            outputScanResult(
              sample,
              secondTotalSize,
              secondGenDeduplicationRatio
            )
        }
    }

    def outputScanResult(
        sample: HarnikEstimationSample,
        totalSize: Long,
        ...
        msg.append("\n")
        msg.append(
          "Harnik's Estimation Results: (based on %s samples)\n".format(
    ...
    def outputNaNWarning(): Unit = {
        val msg = new StringBuffer()
        msg.append(
          "\n\nNote: A NaN entry usually indicates that it was not possible to provide an estimate with a\n"
        )
        msg.append(
          "confidence higher than 99%. Consider increasing the sample size by using --harnik-sample-size\n"
        )
        println(msg)
    }
}

class HarnikEstimationScanHandler(
    val sample: HarnikEstimationSample,
    ...
    val typeMap = mutable.Map.empty[String, HarnikEstimationSampleCounter]
    val sizeCategoryMap =
        mutable.Map.empty[String, HarnikEstimationSampleCounter]
    ...
    override def quit(): Unit = {
        lock.synchronized {
            ...
            HarnikEstimationScanHandler.outputScanResult(
              sample,
              totalSize,
              deduplicationRatio
            )
    ...
    private def getTypeEstimator(
        fileType: String
    ): HarnikEstimationSampleCounter = {
    ...
    private def getSizeCategoryEstimator(
        fileSize: Long
    ): HarnikEstimationSampleCounter = {
    ...
    private def orderingForTypes(
        value: (String, HarnikEstimationSampleCounter)
    ): (Long, String) = {
        if (value._1 == "ALL") {
            return (1L, value._1)
        }
    ...
    private def outputMapToConsole(
        m: mutable.Map[String, HarnikEstimationSampleCounter],
        title: String,
        ord: ((String, HarnikEstimationSampleCounter)) => (Long, String)
    ): Unit = {
