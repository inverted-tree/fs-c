...
class FileDetail {
    ...
    def getCumulatedFileSizeList(l: Array[Long]): Array[Long] = {
...
class FileDetailsHandler(output: Option[String])
    extends FileDataHandler
    with Log {
    var lock: AnyRef = new Object()
    val fileTypeDetailMap = mutable.Map.empty[String, FileDetail]
    val fileSizeDetailMap = mutable.Map.empty[String, FileDetail]
    ...
    private def getFileTypeDetail(t: String): FileDetail = {
    ...
    override def quit(): Unit = {
        output match {
            case None =>
                ...
                val typeDetails =
                    fileTypeDetailMap.toList sortBy (FileDetail.orderingForTypes)
                for ((k, v) <- typeDetails) {
                    println(v.output(k))
                }
                println()
                val sizeDetails =
                    fileSizeDetailMap.toList sortBy (FileDetail.orderingForSizeCategories)
                for ((k, v) <- sizeDetails) {
                    println(v.output(StorageUnit(k.toLong)))
                }
            case Some(filename) =>
                val w = new BufferedWriter(
                  new FileWriter(new java.io.File(filename))
                )
                val typeDetails =
                    fileTypeDetailMap.toList sortBy (FileDetail.orderingForTypes)
                for ((k, v) <- typeDetails) {
