...
case class HarnikReserviorEntry(
    val digest: Digest,
    val chunkSize: Int,
...
/** Handler implementing the sampling phase of Harnik's deduplication estimation
  * approach using Reseviour sampling
  */
class HarnikEstimationSamplingHandler(
    val configuredSampleSize: Option[Int],
    ...
    private def getEstimationSample: HarnikEstimationSample = {
        ...
        val entryMap = mutable.Map.empty[Digest, HarnikSamplingEntry]
        for (entry <- reserviorBuffer) {
            if (entryMap.contains(entry.digest)) {
                ...
                entryMap.update(
                  entry.digest,
                  HarnikSamplingEntry(
                    existingEntry.baseSampleCount + 1,
                    existingEntry.chunkSize
                  )
                )
            } else {
                entryMap += (entry.digest -> HarnikSamplingEntry(
                  1,
                  entry.chunkSize
                ))
            }
