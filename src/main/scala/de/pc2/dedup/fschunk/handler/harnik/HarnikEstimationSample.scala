...
class HarnikEstimationSampleCounter(val sample: HarnikEstimationSample)
    extends Log {
    val map =
        new mutable.OpenHashMap[Digest, Int](sample.totalSampleCount.toInt)
    ...
    def recordNoCheck(fp: Digest, size: Long, isInSample: Boolean): Unit = {
        if (isInSample) {
            map.update(fp, map.getOrElse(fp, 0) + 1)
        }
        totalChunkSize += size
    ...
    lazy val deduplicationRatio: Double = calculateDeduplicationRatio()

    /** Returns the deduplication ratio
      */
    private def calculateDeduplicationRatio(): Double = {

        /** Calculate the number of samples to ensure the confidence in the
          * results. Formulate taken over from the research paper
          */
        ...
        /** We have there if difference to always use the faster way of
          * calculation
          */
        if (sample.map.size < map.size) {
            for ((fp, entry) <- sample.map) {
                if (map.contains(fp)) {
                    sum += (1.0 * entry.baseSampleCount * entry.chunkSize / map(
                      fp
                    ))
        ...
        if (sampleCount < necessaryNumberOfSamples(ratio)) {
            logger.info(
              "Not enough samples: estimated ratio %s, actual samples %s, necessary samples %s"
                  .format(ratio, sampleCount, necessaryNumberOfSamples(ratio))
            )
            logger.debug(
              "Not enough samples: ratio %s, %s/%s, actual samples %s, necessary samples %s"
                  .format(
                    ratio,
                    sum,
                    weightedSampleCount,
                    sampleCount,
                    necessaryNumberOfSamples(ratio)
                  )
            )
            Double.NaN
        } else {
            logger.debug(
              "Ratio %s, %s/%s, actual samples %s, necessary samples %s".format(
                ratio,
                sum,
                weightedSampleCount,
                sampleCount,
