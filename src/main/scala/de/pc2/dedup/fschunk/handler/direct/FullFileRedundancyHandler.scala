...
class FullFileRedundancyHandler() extends FileDataHandler with Log {
    ...
    val fileDigestSet = mutable.Set.empty[Digest]
    ...
    def handle(f: File): Unit = {
        lock.synchronized {
            ...
            val fullFileFingerprint: Digest =
                new Digest(getFileDigestBuilder(f.filename).digest())
    ...
    override def quit(): Unit = {
        ...
        msg.append(
          "\tTotal File Count\t%s\t(%s)\n".format(
            StorageUnit(totalFileCount),
            totalFileCount
          )
        )
        msg.append(
          "\tTotal File Size\t%sB\t(%s)\n".format(
        ...
        msg.append("\n")
        msg.append(
          "\tUnique File Count\t%s\t(%s)\n".format(
            StorageUnit(uniqueFileCount),
            uniqueFileCount
          )
        )
        msg.append(
          "\tUnique File Size\t%sB\t(%s)\n".format(
            StorageUnit(uniqueFileSize),
            uniqueFileSize
          )
        )
        msg.append("\n")
        val deduplicationRatio =
            uniqueFileSize.toDouble / totalFileSize.toDouble
        msg.append("\tDeduplication Ratio\t%s\n".format(deduplicationRatio))
