...
class ZeroChunkDeduplicationHandler() extends FileDataHandler with Log {
    ...
    val zeroIndex = mutable.Set.empty[Digest]
    ...
    def getFullChunkDigest(v: Byte, maxSize: Int, digestSize: Int): Digest = {
        ...
        val rc = new RabinChunker(
          2 * 1024,
          8 * 1024,
          maxSize,
          false,
          new DigestFactory("SHA-1", digestSize, None),
          "c8"
        )
    ...
    private def outputMapToConsole(): Unit = {
        lock.synchronized {
            ...
            println(
              "Total size: %s, %s".format(
                storageUnitIfPossible(totalSize),
                totalSize
              )
            )
            println(
              "Redundant size: %s, %s".format(
                storageUnitIfPossible(redundantSize),
                redundantSize
              )
            )
