...
/** Read to be able to parse the old legacy files that have been created using
  * the C version of FS-C.
  */
class LegacyFormatReader(file: InputStream, receiver: FileDataHandler)
    extends Reader
    with Log {
    var fileCount = 0L
    var chunkCount = 0L
    ...
    @tailrec
    private def parseFileEntry(reader: BufferedReader): Unit = {
    ...
    def parse(): Unit = {
        try {
            val reader = new BufferedReader(
              new InputStreamReader(file, Charset.forName("ISO-8859-1"))
            );
            try {
                parseFileEntry(reader)
            } catch {
...
object LegacyFormat extends Format {
    def createReader(file: InputStream, receiver: FileDataHandler) =
        new LegacyFormatReader(file, receiver)
    def createWriter(file: OutputStream, privacyMode: PrivacyMode) =
        new LegacyFormatWriter()
}
