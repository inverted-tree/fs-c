...
object JavaVersionDetector extends Log {
    ...
    private var runsOnJava7Cached: Boolean = false

    def runsOnJava7(): Boolean = {
        if (!detected) {
            logger.debug(
              "Java version %s".format(System.getProperty("java.version"))
            )
...
object OSDetector {
    private var detected: Boolean = false
    private var runsOnWindowsCached: Boolean = false
...
object DirectoryProcessor extends Log {
    ...
    /** Portable, but slow for very large directories directory listener. Calls
      * the handler for every file
      */
    private def listDirectoryPortable(
        directory: File,
        handler: (File) => Unit
    ): Long = {
    ...
    private def listDirectoryJava7(
        directory: File,
        handler: (File) => Unit
    ): Long = {
    ...
    /** Linux specific directory listener
      */
    private def listDirectoryLinux(
        directory: File,
        handler: (File) => Unit
    ): Long = {
        var p: Process = null
    ...
    /** Initializes the object
      */
    def init(d: FileDispatcher, useJavaDirectoryListing: Boolean): Unit = {
        synchronized {
...
/** Processes the directory tree. If this finds a file (not a directory) it
  * forwards the file to the dispatcher for chunking
  */
class DirectoryProcessor(
    ...
    val ignoreSet: Set[String] = Set("/dev", "/proc/")
    ...
    private def processFile(file: File): Unit = {
        ...
        if (!followSymlinks && cp != ap) {
            ...
            if (file.isFile) {
                DirectoryProcessor.dispatcher.dispatch(
                  file,
                  cp,
                  isDir = false,
                  source,
    ...
    def run(): Unit = {
        ...
        try {
            dirEntryCount =
                DirectoryProcessor.directoryLister(directory, processFile)
        } catch {
        ...
        logger.debug(
          "Finished Directory %s: time %sms, %s files".format(
            directory,
            diffMillis,
            dirEntryCount
          )
