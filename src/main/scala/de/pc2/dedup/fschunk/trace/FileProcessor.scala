...
object FileProcessor extends Log {
    ...
    private val MAX_CHUNKLIST_SIZE = 10000L
    private val defaultBufferSize: Long = 4L * 1024 * 1024
    private val directBufferThreshold: Long = 4L * 1024 * 1024 * 1024

    def report(): Unit = {
        logger.info(
          "IO Stats: read: %s ops, %s bytes".format(
            totalCount,
            StorageUnit(totalRead.longValue())
          )
        )
    }
    ...
    def init(
        c: Seq[(Chunker, List[FileDataHandler])],
        p: ((File) => Unit),
        useRelPath: Boolean
    ): Unit = {
        synchronized {
...
class FileProcessor(
    ...
    private def readIntoBuffer(
        c: FileChannel,
        buffer: ByteBuffer,
        offset: Long
    ): Int = {
        logger.debug(
          "File: Read %s, offset %s".format(file, StorageUnit(offset))
        )
        val r = c.read(buffer)
        logger.debug(
          "File: Read %s finished, offset %s, data size %s".format(
            file,
            StorageUnit(offset),
            StorageUnit(r)
          )
    ...
    private def filenameToStore(): String = {
        ...
        if (!FileProcessor.useRelativePaths) {
            path
        } else {
            sourcePath match {
                case None => path
    ...
    def run(): Unit = {
        ...
        FileProcessor.synchronized {
            while (
              FileProcessor.chunker == null || FileProcessor.progressHandler == null
            ) {
                FileProcessor.wait()
            }
        }
        ...
        logger.debug(
          "Started File %s (%s)".format(file, StorageUnit(fileLength))
        )
        try {
            logger.debug("File %s: Allocate buffer".format(file))
            val buffer = allocateBuffer(fileLength)
            val sessionList =
                for { chunker <- FileProcessor.chunker } yield (
                  chunker._1.createSession(),
                  chunker._2,
                  new ListBuffer[Chunk]()
                )
            ...
            while (r > 0) {
                ...
                for ((session, handlers, chunkList) <- sessionList) {
                    ...
                    if (chunkList.size > FileProcessor.MAX_CHUNKLIST_SIZE) {
                        val fp =
                            new FilePart(filenameToStore(), chunkList.toList)
                ...
                logger.debug(
                  "File %s: Chunk finished, time %sms".format(
                    file,
                    (endChunkMillis - startChunkMillis)
                  )
                )
            ...
            val fileWithoutChunks =
                new File(filenameToStore(), fileLength, t, List(), label)
        ...
        logger.debug(
          "Finished File %s: time %sms, size %s".format(
            file,
            diffMillis,
            StorageUnit(fileLength)
          )
        )
