...
/** A rolling fingerprint variant of Rabin's Fingerprinting Method. It used a
  * "invert table" to speed up the processing.
  *
  * A proof for the correctness of the invert table can be found in "Dirk
  * Meister, Data Deduplication using Flash-based indexes, Master thesis,
  * University of Paderborn, 2009"
  */
class RabinWindow(rabin: Rabin, windowSize: Int) extends RollingFingerprint {
    val invertTable: Array[Long] = createInvertTable(rabin, windowSize)

    def createInvertTable(rabin: Rabin, windowSize: Int): Array[Long] = {

        /** To extract and old byte i from the rabin fingerprint i * 1 << (63 *
          * 8 + 1) (correct? Double check in thesis) must be "and"-ed to the
          * current fingerprint.
          */
        @tailrec
        def calculateWindowSizeShift(i: Int, s: Long): Long = {
            if (i < windowSize)
                calculateWindowSizeShift(i + 1, rabin.append(s, 0.toByte))
            else s
        }
        ...
        val it =
            for (i <- 0 until 256)
                yield PolynomUtil.modmult(i, shift, rabin.polynom)
        it.toArray
    }

    def printInvertTable(): Unit = {
        for (i <- 0 until 256) {
    ...
    class RabinWindowSession extends Session with Log {
        ...
        def append(data: Int): Unit = {
            ...
            /** Removed oldest byte from fingerprint and adds new byte (data) to
              * it
              */
            fingerprint =
                rabin.append(fingerprint ^ invertTable(oldestByte), data)
        }
    }
}
