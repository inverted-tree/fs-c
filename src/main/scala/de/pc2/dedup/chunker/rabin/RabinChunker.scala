...
class RabinChunker(
    ...
    /** breakmark bit pattern
      */
    private val breakmark: Long =
        (pow(2.0, BigInteger.valueOf(averageSize).bitLength() - 1) - 1).toLong
    ...
    private class RabinChunkerSession(val chunkername: String)
        ...
        /** Also, adds the rabin fingerprint (hash) to the chunk. Usually the
          ...
          * case when the chunk has been closed due to size limitations
          */
        def acceptChunk(
            h: (Chunk => Unit),
            nonChunkedData: ByteBuffer
        ): Unit = {
            ...
            val c = if (nonChunkedData != null) {
                ...
                val digest = digestBuilder
                    .append(overflowChunk, 0, overflowChunkPos)
                    .append(nonChunkedData)
                    .build()
        ...
        def debugString(b: ByteBuffer): String = {
            return "position %s, limit %s, capacity %s".format(
              b.position(),
              b.limit(),
              b.capacity()
            )
                ...
                while (current < data.limit() && !break) {
                    ...
                    if (countToMin > todo) {
                        ...
                        while (current < end && !innerBreak) {
                            ...
                            val isBreakmark =
                                (this.rabinSession.fingerprint & breakmark) == breakmark;
                            if (isBreakmark) {
