...
trait RollingFingerprint {
    ...
    /** Rolling fingerprint session. A session contains the current state of a
      * fingerprint and should only be called from a single thread.
      */
    trait Session {
        ...
        /** Appends a new data element. Throws IllegalArgumentException if data
